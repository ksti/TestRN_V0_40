 ...
 * 
 <StepView
    ...
    DefaultViewImgUrl={require('../../../images/App/ic_flow_finished.png')}
    CompletedViewTvStyle={{color: '#36BC99',marginRight:5,marginLeft:5}}
    CompletedViewImgUrl={require('../../../images/App/ic_gray_rightArrow.png')}
    SelectViewTvStyle={{color: '#FF5001',marginRight:5,marginLeft:5}}
    SelectViewImgUrl={require('../../../images/App/ic_flow_finished.png')}
...
export default class StepView extends Component {
    ...
    _renderTypeOne = () => {
        return (
            <View  {...this.props}
                style={[this.props.style, this.props.bgstyle]}
                >
                <ScrollView
                    automaticallyAdjustContentInsets={false}
                    horizontal={true}
                    style={[{ overflow: 'hidden' }]}
                    contentContainerStyle={[{ justifyContent: 'center', backgroundColor: 'transparent', alignItems: 'center' }, this.props.contentStyle]}
                    showsHorizontalScrollIndicator={false}
                    showsVerticalScrollIndicator={false}
                    removeClippedSubviews={true}
                    >
                    {this.getItemsOne(this.props.dataSource)}
                </ScrollView>
            </View>
        );
    }
        ...
        return (
            <View  {...this.props}
                ...
                <ScrollView
                    ...
                    horizontal={true}
                    style={[{ overflow: 'hidden' }]}
                    contentContainerStyle={[{ justifyContent: 'center', backgroundColor: 'white', alignItems: 'center' }, this.props.contentStyle]}
                    showsHorizontalScrollIndicator={false}
                    showsVerticalScrollIndicator={false}
                    removeClippedSubviews={true}
                    scrollEnabled={this.props.dataSource.length == 1 ? false : true}
                    >
                    {this.getItemsTwo(this.props.dataSource)}
                </ScrollView>
            </View>
        );
    }
        ...
        return (
            <View  {...this.props}
                ...
                <ScrollView
                    ...
                    removeClippedSubviews={true}
                    >
                    {this.getItemsThree(this.props.dataSource)}
                </ScrollView>
    ...
    getItemsOne = (rowData) => {
        ...
        for (let i = 0; i < rowData.length; i++) {
            ...
            if (this.props.nodeState) {
                Completedindex = this.props.nodeState;
            }
    ...
    getItemsTwo = (rowData) => {
        ...
        for (let i = 0; i < rowData.length; i++) {
            ...
            if (Completedindex === undefined) {//如果没有返回完成到那个节点 就返回一个节点的数据 在数据源查找位置  用来判断已经完成的节点的位置                
                ...
                if (this.props.click) {
                    ...
                    viewArr.push(
                        <View key={i} style={{ flexDirection: 'row' }}>
                            ...
                            <View style={[styles.rowtwo, rowData.length == 1 ? styles.placeholderDisplay : styles.placeholderNoDisplay]}>
                                <View style={{ height: 20, justifyContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
                                    <View style={[{ flex: 1, height: 1 }, lineColor]} />
                                </View>

                                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
                                    <Text style={{ color: 'transparent' }}>空白占位符空白占位符空白占位符空白占位符空白占位符空白占位符
                                    空白占位符空白占位符空白占位符空白占位符空白占位符空白占位符空白占位符空白占位符空白占位符空白占位符空白占位符空白占位符
                                    </Text>
                                </View>
                            </View>
    ...
    getItemsThree = (rowData) => {
        ...
        for (let i = 0; i < rowData.length; i++) {
            ...
            let propsData = this.props.propsData;
            let collorArr = ['#FF5001', 'yellow'];
            let processStatus = propsData.processStatus;
            let pointHeight = 10;
            if (propsData.isAbarbeitung == false) {
                pointHeight = 1;
            }

            if (processStatus == 0 && propsData.isAcceptancelink) {
                collorArr = [this.props.SelectViewTvStyle.color, this.props.SelectViewTvStyle.color];
            } else if (processStatus == 0 && propsData.isAcceptancelink == false) {
                collorArr = ['#FF5001', 'yellow'];
            } else {
                pointHeight = 10;
                collorArr = [this.props.CompletedViewIndicatorStyle.backgroundColor, this.props.CompletedViewIndicatorStyle.backgroundColor];
            }
            ...
            if (i === 0) {
                    ...
                    viewArr.push(
                        <TouchableOpacity onPress={() => { this.click(rowData[i]) } } underlayColor='transparent' key={i} >
                            <View style={[styles.rowtwo, this.props.rowStyle]}>
                                <View style={{ height: 20, justifyContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
                                    <View style={{ flex: 1, height: 1, backgroundColor: 'white', }} />
                                    <View style={[{ height: 10, width: 5, borderTopLeftRadius: 5, borderBottomLeftRadius: 5, backgroundColor: processStatus > i ? CompletedViewIndicatorStyle.backgroundColor : collorArr[0] }]} />
                                    <View style={[{ height: 10, width: 5, borderTopRightRadius: 5, borderBottomRightRadius: 5, backgroundColor: processStatus > i ? CompletedViewIndicatorStyle.backgroundColor : collorArr[1] }]} />
                                    <View style={[{ flex: 1, height: 1 }, lineColor]} />
                                </View>

                                <View
                                    style={{ flex: 1, justifyContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
                                    <Text style={DefaultViewTvStyle}>
                                        {rowData[i]}
                                    </Text>
                                </View>
                ...
                if (processStatus == 3 && propsData.isReinspectionlink) {
                    collorArr = [this.props.SelectViewTvStyle.color, this.props.SelectViewTvStyle.color];
                } else if (processStatus == 3 && propsData.isReinspectionlink == false) {
                    collorArr = ['#FF5001', 'yellow'];
                } else {
                    collorArr = [this.props.DefaultViewIndicatorStyle.backgroundColor, this.props.DefaultViewIndicatorStyle.backgroundColor];
                }
                        ...
                        <TouchableOpacity onPress={() => { this.click(rowData[i]) } } underlayColor='transparent' key={i}>
                            <View style={[styles.rowtwo, this.props.rowStyle]}>
                                <View
                                    style={{ height: 20, justifyContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
                                    <View style={[{ flex: 1, height: 1 }, lineColor]} />
                                    <View style={[{ height: pointHeight, width: 5, borderTopLeftRadius: 5, borderBottomLeftRadius: 5, backgroundColor: processStatus > i ? CompletedViewIndicatorStyle.backgroundColor : collorArr[0] }]} />
                                    <View style={[{ height: pointHeight, width: 5, borderTopRightRadius: 5, borderBottomRightRadius: 5, backgroundColor: processStatus > i ? CompletedViewIndicatorStyle.backgroundColor : collorArr[1] }]} />
                                    <View style={{ flex: 1, height: 1, backgroundColor: 'white', }} />
                                </View>
                                <View
                                    style={{ flex: 1, justifyContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
                        ...
                        <TouchableOpacity onPress={() => { this.click(rowData[i]) } } underlayColor='transparent' key={i}>
                            <View style={[styles.rowtwo, this.props.rowStyle]}>
                                <View
                                    ...
                                    <View style={[{ flex: 1, height: 1 }, lineColor]} />
                                    <View
                                        style={[{ height: pointHeight, width: 10, borderRadius: 5 }, processStatus > i ? CompletedViewIndicatorStyle : processStatus == i ? SelectViewIndicatorStyle : DefaultViewIndicatorStyle]} />
                                    <View style={[{ flex: 1, height: 1 }, lineColor]} />
                                </View>
                                <View
                                    style={{ flex: 1, justifyContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
