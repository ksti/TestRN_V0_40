...
var SQLTransation ={
    ...
    */
    deleteTable(tableName:string,callback:Function){
        ...
        db.executeSql(sqlDrop,[],callback && callback(true,''),(error)=>callback && callback(false,error.message));
    },
    /*查表:返回表内所有数据
        selectSQL:查表语句
    */
    ...
    insertData(tableName: String, data: Object,callback: Function){
        ...
        for (var key in data) {
            ...
            if ( valueType === 'string' ) {
                ...
                let jsonObject = data[key];
                let jsonString = JSON.stringify(jsonObject);
                valueString = valueString + '\'' + jsonString + '\',';
            } else {
                valueString = valueString+data[key]+',';
            }
        }
    ...
    */
    ...
    managerSQL(sqlString:string,callback:Function){
        if (!db) {
            callback && callback(false,'数据库不存在');
            return;
        }
